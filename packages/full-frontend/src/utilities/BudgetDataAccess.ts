...
export class BudgetDataAccess {
  ...
  static async fetchBudgetsForSelectedSim(
    userSimulation: string
  ): Promise<Budget[]> {
    // fetch inputs.
    ...
    try {
      const response = (await API.graphql({
        ...
        variables: { simulation: userSimulation, nextToken: nxtTkn },
      })) as { data: BudgetsBySimulationIdQuery };
      for (const budget of response.data.budgetsBySimulationId!.items!) {
  ...
  static async fetchDefaultBudgets(
    ...
    try {
      ...
      for (const budget of response.data.listBudgets!.items!) {
        if (
          budget?.simulation &&
          budget?.simulation! === selectedSimulationId
        ) {
          let cats: Category[] = [];
          for (const category of budget.categories || []) {
            if (category) {
