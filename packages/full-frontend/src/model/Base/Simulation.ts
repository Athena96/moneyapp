...
export class Simulation implements Key {
    ...
    getSimulationData() {
        ...
        for (const item of jsonData) {
            ...
            const utcDate = new Date(year, 1, 1);
            ...
            if (item.events) {
                ...
                for (const event of item.events!) {
                    d.events.push(new Event(event.id, event.name, event.date, new Category(event.category.id, event.category.name, event.category.value), event.type, this.id))
                }
            }
